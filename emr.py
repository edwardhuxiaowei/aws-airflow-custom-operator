...
class EmrCustomOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        emr_step_sensor = EmrStepSensor(
            ...
            job_flow_id=job_flow_id,
            step_id="{{task_instance.xcom_pull('add_steps_task', key='return_value')[0]}}",
            aws_conn_id=self.aws_conn_id,
            mode='poke',
        )

        emr_add_steps_operator.execute(context)
        emr_step_sensor.execute(context)